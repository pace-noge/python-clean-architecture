...
def test_repository_list_with_code_equal_filter(room_dicts):
    ...
    room = repo.list(
        filters={"code__eq": room_dicts[0].get("code")}
    )

    assert len(room) == 1
    assert room[0].code == room_dicts[0].get("code")


@pytest.mark.parametrize("price", [60, "60"])
def test_repo_list_with_price_equal_filter(room_dicts, price):
    repo = MemRepo(room_dicts)
...
def test_repository_list_filter_with_price_between_filter(room_dicts):
    repo = MemRepo(room_dicts)
